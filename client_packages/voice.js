...
mp.events.add('v_reload', () => {
	try {
		if(g_voiceMgr.listeners.length != 0) {
			let i = 0;
			let players = g_voiceMgr.listeners.length;
			for (i = 0; i < players; i++) {
				if(i < g_voiceMgr.listeners.length) g_voiceMgr.remove(g_voiceMgr.listeners[i], true); // На всякий случай еще раз проверка, на всякий, от случайных.
			}
		}
	} catch { }
});

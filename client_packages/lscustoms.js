...
mp.events.add('tclk', (id) => {
    ...
    if (setted)
        ...
        if (lscPage === "paint_menu_one" || lscPage === "paint_menu_two")
            mp.events.call("hideColorp");

        var title = (lscPage === "paint_menu_one" || lscPage === "paint_menu_two") ? "Вы действительно хотите покрасить машину в данный цвет?" : "Вы действительно хотите установить данную модификацию?";
...
mp.events.add('testmodtype', function (id) {
    mp.gui.chat.push(`Primary: ${localplayer.vehicle.getCustomPrimaryColour(1, 1, 1)}`);
    mp.gui.chat.push(`Secondary: ${localplayer.vehicle.getCustomSecondaryColour(1, 1, 1)}`);

    var num = localplayer.vehicle.getNumMods(id);
    if (num > 0)
        mp.gui.chat.push(`${id} available: ${num}`);
    else mp.gui.chat.push(`${id} isnt available`);
});

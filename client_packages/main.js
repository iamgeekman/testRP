...
mp.keys.bind(Keys.VK_Z, false, function () { // Z key
    if (!loggedin || chatActive || editing || new Date().getTime() - lastCheck < 3000 || global.menuOpened) return;
	if(localplayer.vehicle) {
...
mp.events.add('syncWP', function (bX, bY) {
    if(!mp.game.invoke('0x1DD1F58F493F1DA5')) {
		mp.game.ui.setNewWaypoint(bX, bY);
		mp.events.call('notify', 2, 9, "Пассажир передал Вам информацию о своём маршруте!", 3000);
	}else mp.events.call('notify', 4, 9, "Пассажир попытался передать Вам информацию о маршруте, но у Вас уже установлен другой маршрут.", 5000);
});

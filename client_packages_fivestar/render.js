...
function getLookingAtEntity() {
    ...
    let secondPoint = mp.game.graphics.screen2dToWorld3d([res.x / 2, res.y / 2, (2 | 4 | 8)]);
    ...
    if (typeof result !== 'undefined') {
        ...
        if (result.entity.type == 'object' && (result.entity.getVariable('TYPE') == undefined && !result.entity.isFurniture)) return null;
    ...
    if (localPlayer.isInAnyVehicle(false)) {
        ...
        objects.forEach(
            (object) => {
                ...
                if (object.getVariable('TYPE') != undefined || object.isFurniture) {
...
let lastDimension = 0;
let lastPlayerPosition = new mp.Vector3(0, 0, 0);

global.helpText = "";

mp.events.add('render', () => {
    try {
        if (mp.game.ui.isPauseMenuActive() && mp.gui.cursor.visible) mp.gui.cursor.visible = false;

        if (!loggedin) return;

        if (lastDimension != localPlayer.dimension) {
            lastDimension = localPlayer.dimension;
            mp.events.call('dimensionChange');
        }

        if (mp.game.gameplay.getDistanceBetweenCoords(lastPlayerPosition.x, lastPlayerPosition.y, lastPlayerPosition.z, localPlayer.position.x, localPlayer.position.y, localPlayer.position.z, true) > 100) {
            mp.events.call('teleportTrigger');
        }
        lastPlayerPosition = localPlayer.position;

        if (localPlayer.isSprinting()) mp.game.player.restoreStamina(100);
        ...
        if (global.helpText != "") {
            mp.game.graphics.drawText(global.helpText, [0.5, 0.9], {
                font: 0,
                color: [255, 64, 64, 255],
                scale: [0.5, 0.5],
                outline: true
